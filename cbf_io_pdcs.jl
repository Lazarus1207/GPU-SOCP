# cbf_io_pdcs.jl
#
# PDCS-only fork of HPRSOCP/src/utils/cbf_io.jl with EXP cone support added.
# Do NOT use this from HPR / PDHCG solvers — they have their own copy.
#
# Differences from the original:
#   - _cbf_is_supported_cone() also accepts "EXP" and "EP"
#   - _cbf_prepare_layout() returns EXP_con_idx (similar to SOC_con_idx)
#   - read_cbf() / _read_cbf_io() / _read_cbf_mmap() return EXP_con_idx
...
@inline _cbf_is_supported_cone(cone_type::AbstractString) =
    cone_type == "L+" || cone_type == "L-" || cone_type == "L=" || cone_type == "Q" ||
    cone_type == "F" || cone_type == "EXP" || cone_type == "EP"
...
function _cbf_prepare_layout(con::Vector{Tuple{String,Int}}, var::Vector{Tuple{String,Int}}; nconstr::Int=-1, nvar::Int=-1)
    ...
    for (cone_type, len) in con
        ...
        elseif cone_type == "EXP" || cone_type == "EP"
            number_exp_rows += len
            push!(exp_con_lens, len)
        end
    end

    total_rows = number_eq + number_ineq + number_soc_rows + number_exp_rows
    row_new = zeros(Int, m)
    ...
    exp_ptr = number_eq + number_ineq + number_soc_rows + 1
    r0 = 1
    for (cone_type, len) in con
        r1 = r0 + len - 1
        if cone_type == "L="
        ...
        while true
            ...
            if line == "CON"
                ...
                if !layout_ready && !isempty(var)
                    ...
                    EXP_con_idx = layout.EXP_con_idx
